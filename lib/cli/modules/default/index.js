...
exports.init = function (api) {
    ...
    //delete the "sample" part from the copied js files in etc
    const serverJsPath = path.join(etcFolder, 'server.js.sample.js');
    const routerJsPath = path.join(etcFolder, 'routing.js.sample.js');
    return new Promise((resolve, reject) =>{
        
        fs.move(routerJsPath, path.join(etcFolder, 'routing.js'), err => {
            if (err) return reject('err');
            resolve(); 
        });

        fs.move(serverJsPath, path.join(etcFolder, 'server.default.js'), err => {
            if (err) return reject('err');
            resolve(); 
        });
