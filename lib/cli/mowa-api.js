...
module.exports = class MowaAPI {
    constructor(modules) {
        ...
        this.argv = optimist
            .usage('Usage: $0 [module] <command> [--env=<target environment>] [--mod=<target web module>]\n    modules: ' + moduleNames)
            .options('e', {
                alias : 'env',
            ...
            .alias('m', 'mod')
            .describe('m', 'only process specified target web module')
            .argv;

        let env = this.getOption('env');
        ...
        let firstArg = inflection.camelize(args[0], true);

        let taskModuleName = 'default';
        let taskModule = modules[taskModuleName];
        if (modules[firstArg]) {
            taskModuleName = firstArg;
            taskModule = modules[firstArg];
            args.splice(0, 1);
        }

        this.taskModuleName = taskModuleName;
        this.command = args.shift();
        this.commandHandler = taskModule[this.command];
        
        if (!this.commandHandler || typeof this.commandHandler !== 'function') {
            console.error(`unknown command ${this.command}`);
            this.showUsage();
            process.exit(1);
        }

        const fileName = 'deploy.' + env + '.js';
        ...
        try {
            ...
            if (e.code == 'MODULE_NOT_FOUND') {
                ...
                if (this.taskModuleName !== 'default' || this.command !== 'init') {                   
    ...
    runCommand() {
        return Promise.resolve(this.commandHandler(this));        
    }    
    ...
    getConfig(moduleName) {
        return this.config && this.config[moduleName];
    }
